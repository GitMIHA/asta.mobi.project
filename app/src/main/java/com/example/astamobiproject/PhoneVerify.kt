    ...
    private fun signInWithPhoneAuthCredential(credential: PhoneAuthCredential) {
        auth.signInWithCredential(credential)
            .addOnCompleteListener(this) { task ->
                ...
                // Sign in failed, display a message and update the UI -- Помилка входу, відображення повідомлення та оновлення інтерфейсу користувача
                    if (task.exception is FirebaseAuthInvalidCredentialsException) {
                // The verification code entered was invalid --Введений код підтвердження недійсний
